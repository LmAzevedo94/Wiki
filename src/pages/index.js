...
function DocsSidebar() {
  return (
    <aside className={styles.sidebar}>
      <p className={styles.sidebarHeading}>Documentação</p>
      {sidebarLinks.map((group) => (
        <div key={group.category} className={styles.sidebarGroup}>
          <p className={styles.sidebarCategory}>{group.category}</p>
          <ul className={styles.sidebarList}>
            {group.links.map((link) => (
              <li key={link.to}>
                <Link to={useBaseUrl(link.to)} className={styles.sidebarLink}>
                  {link.label}
                </Link>
              </li>
            ))}
